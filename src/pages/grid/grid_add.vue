      ...
      <Row type="flex" :gutter="15">
        ...
        <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="100">
          <FormItem label="餐柜名称:" prop="buffet_name">
            <Input clearable v-model="formValidate.buffet_name" placeholder="请输入餐柜名称"></Input>
          </FormItem>
          <FormItem label="餐柜编号:" prop="buffet_code">
            <Input clearable v-model="formValidate.buffet_code" placeholder="请输入餐柜编号"></Input>
          </FormItem>
          <FormItem label="餐柜地址:" prop="address">
            <Input clearable v-model="formValidate.address" placeholder="请输入餐柜地址"></Input>
          </FormItem>
          <FormItem>
        ...
        <Col :md="12" :lg="12" :xs="24" :sm="24">
        <baidu-map v-bind:style="mapStyle" zIndex="1" class="bm-view" :center="center" :zoom="zoom" :scroll-wheel-zoom="true">
          <bm-view style="width: 100%; height:500px;"></bm-view>
          <bm-marker massClear :position="{lng: center.lng, lat: center.lat}" animation="BMAP_ANIMATION_BOUNCE">
          </bm-marker>
          <bm-control :offset="{width: '10px', height: '10px'}">
            <bm-auto-complete v-model="formValidate.address" :sugStyle="{zIndex: 999999}">
              <input type="text" v-model="formValidate.address" placeholder="请输入搜索关键字" class="serachinput">
            </bm-auto-complete>
          </bm-control>
          <bm-local-search :keyword="formValidate.address" :auto-viewport="true" style="width:0px;height:0px;overflow: hidden;"></bm-local-search>
        </baidu-map>
        </Col>
      </Row>
    </div>
    ...
    loadPage() {
      ...
      if (this.$route.query && this.$route.query.buffet_id) {
        this.formValidate = {
          ...
          buffet_code: this.$route.query.buffet_code
        }
        this.center = {
    ...
    async register() {
      ...
      if (this.$route.query && this.$route.query.buffet_id) {
        ...
        res = await buffet_edit({
          ...
          buffet_code: this.formValidate.buffet_code,
          lng: this.center.lng,
          lat: this.center.lat
        })
      } else {
