...
<template>
  ...
  <div class="users_list">
    <Card class="vue-panel">
      <p slot="title">
        条件查询
        <router-link to="/answer/answer_add">
          <Button icon="plus" class="fr vue-back-btn header-btn" type="primary" size="small">添加答题</Button>
        </router-link>
      </p>
      <Form ref="formItem" :model="formItem" :label-width="100" :rules="ruleValidate">
        <Row>
          ...
          <FormItem label="答题ID:" prop="id">
            <Input clearable v-model="formItem.id" placeholder="请输入答题ID" clearable></Input>
          ...
          <Col :xs="24" :sm="24" :md="8" :lg="8" span="8">
          <FormItem label="答题标题:">
            <Input clearable v-model="formItem.title" placeholder="请输入答题标题" clearable></Input>
          </FormItem>
          </Col>
          <Col :xs="24" :sm="24" :md="8" :lg="8" span="8">
          <FormItem label="状态:">
    ...
    return {
      ...
      tableColumns: [
        ...
        },
        {
          ...
          render: (h, params) => {
            ...
            params.row.options.forEach(element => {
              if (params.row.result + "" === element.id + "") {
                name = element.name
              }
            })
            return h("span", [
        ...
        },
        {
          ...
          render: (h, params) => {
            const row = params.row
            const status = filters.is_yes(Number(row.status))
            return h("span", [
              h(
                "Tooltip",
                {
